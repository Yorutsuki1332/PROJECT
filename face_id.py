...
class EnhancedFaceID:
    ...
    def register_face(self, user_id, frame):
        ...
        return True, f"Face registered successfully. {remaining_slots} slots remaining"

    def identify_face(self, frame):
        """Returns (is_match, user_id)"""
...
def main():
    ...
    while True:
        ...
        if key == ord('q'):
            ...
            if is_match:
                print(f"Face matched! User: {matched_user}")
                draw_face_info(display_frame, face_rect, f"Matched: {matched_user}")
            else:
                print("No match found")
                draw_face_info(display_frame, face_rect, "No match")
            cv2.imshow('Enhanced Face ID System', display_frame)
            cv2.waitKey(1000)
        elif key == ord('l'):
